...
func (e *FileEvent) IsModify() bool {
	return (e.IsFileWrite() || (e.mask&sys_IN_ATTRIB) == sys_IN_ATTRIB)
}

// IsFileWrite reports whether the FileEvent was triggered by a file modification
			...
			if !event.ignoreLinux() {
				...
				if _, fsnFound := w.fsnFlags[event.Name]; !fsnFound {
					if fsnFlags, watchFound := w.fsnFlags[watchedName]; watchFound {
						...
						w.fsnFlags[event.Name] = ALL_FLAGS
					}
				}
				w.fsnmut.Unlock()
